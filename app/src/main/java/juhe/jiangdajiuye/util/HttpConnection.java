 ...
 *
 * 考虑到 解析网页需要耗时，
 * 所以直接将网页解析工作放在这里，直接返回MessageItem 对象
 */
public class HttpConnection {
    ...
    int ResponseCode = 0;
    public  void realGet(String url1,final XuanJiangMesHolder holder ,final int tab){
        result.setLength(0);
        ...
        try{
            ...
            if(ResponseCode==200){
               final List<MessageItem> data = parseResponse(result.toString(),holder,tab);
                ((Activity)mCtx).runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if(data.size() ==0){
                            ...
                            lister.success(data,ResponseCode);
                        }
                    }
                });
            }
    ...
    public List<MessageItem> parseResponse(String response,XuanJiangMesHolder holder,int tab){
        if(null != holder){
            //解析各个学校的宣讲会
